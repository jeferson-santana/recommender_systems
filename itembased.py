...
if M > 2000:
    print("N =", N, "are you sure do you want to continue?")
...
def predict(u, m):
    ...
    # if item has no neighbors, fallback to its mean
    if m >= len(neighbors):
        return averages[m]

    for neg_w, j in neighbors[m]:   # neighbors by ITEM
        w = -neg_w
        try:
            numerator   += w * deviations[j][u]  # user u's deviation on neighbor item j
            denominator += abs(w)
        except KeyError:
            # user u didn't rate item j
            continue

    prediction = averages[m] if denominator == 0 else (averages[m] + numerator / denominator)
...
for (u, m), target in usermovie2rating.items():
    # calculate the predicition for this movie
    prediction = predict(u, m)

    # save the predicition and target
